...
@RuleNode(
        ...
        nodeDescription = "发布消息到Kafka服务器",
        nodeDetails = "通过Kafka生产者将记录发布到Kafka服务器。" +
                "出站消息将包含消息元数据中的Kafka响应字段(<code>offset</code>, <code>partition</code>, <code>topic</code>)" +
                "，例如可以通过<code>metadata.partition</code>访问<b>partition</b>字段。",
